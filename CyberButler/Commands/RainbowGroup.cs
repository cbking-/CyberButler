...
{
    [Group("rainbow")]
    internal class RainbowGroup : BaseCommandModule
    {
        ...
        public async Task Toggle(CommandContext _ctx)
        {
            ...
            var rainbowRole = _ctx.Guild.Roles.FirstOrDefault(
                role => role.Value.Name == Configuration.Config["RainbowRole"]);
            ...
            else
            {
                ...
                while (File.Exists("rainbow"))
                {
                    ...
                    {
                        await rainbowRole.Value.ModifyAsync(color: colorEnum.Current);
